...
void default_constants(){
  ...
  chassis.set_drive_constants(10, 1.5, 0, 10, 0); // lateral PID tuning

  chassis.set_heading_constants(6, .4, 0, 1, 0);  // used for drive-to-point and heading maintaining


  chassis.set_turn_constants(12, 0.4, 0, 0, 15); // angular PID constants (MaxVoltage, kP, kI, kD, startI).

  chassis.set_swing_constants(12, .3, .001, 2, 15); // used for swing turns (one side still, one side moves)

  // Each exit condition set is in the form of (settle_error, settle_time, timeout).
  chassis.set_drive_exit_conditions(1.5, 300, 5000); // desides when it is "close enough" to the target and stops
